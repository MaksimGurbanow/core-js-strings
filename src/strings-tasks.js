...
function isPalindrome(str) {
  ...
  let temp = 0;
  while (temp < Math.floor(formattedStringLength / 2)) {
    if (
      formattedString.charCodeAt(temp) !==
      formattedString.charCodeAt(formattedStringLength - 1 - temp)
    ) {
      return false;
    }
    temp += 1;
